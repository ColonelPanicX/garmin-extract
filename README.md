...
- **"failed to close window in 20 seconds" persists after clearing orphaned processes and Singleton files** — the browser profile may have accumulated corrupt internal state (preferences, cached session data) from repeated crash/kill cycles. Fix: `rm -rf .garmin_browser_profile/` to wipe the profile entirely. The next run will do a fresh login and MFA — credentials are unaffected.
