...
def extract_day(path: Path, lifestyle_behaviors: dict = None) -> dict:
    ...
    floors_up = (
        round(float(st["floorsAscendedInMeters"]), 1)
        if st.get("floorsAscendedInMeters") is not None
        else None  # noqa: E501
    )
    floors_down = (
        round(float(st["floorsDescendedInMeters"]), 1)
        if st.get("floorsDescendedInMeters") is not None
        else None  # noqa: E501
    )
    ...
    sleep_score = s(
        scores_src.get("overallScore")
        or (_overall.get("value") if isinstance(_overall, dict) else _overall)
    )
    ...
    tr_load = s(
        (ts.get("mostRecentTrainingLoadBalance") or {}).get("monotonicLoad")
        or (ts.get("mostRecentTrainingLoadBalance") or {}).get("acuteLoad")
    )
        ...
        rows.append(
            {
                ...
                "Activity Type": (  # noqa: E501
                    a.get("activityTypeDTO", {}).get("typeKey")
                    if isinstance(a.get("activityTypeDTO"), dict)
                ...
                "Distance (miles)": meters_to_miles(
                    (a.get("distance") or 0) / 100
...
def main():
    ...
    parser.add_argument(
        "--since", metavar="YYYY-MM-DD", help="Only include dates on or after this date"
    )
    args = parser.parse_args()

    since = args.since or "2000-01-01"

    files = sorted(p for p in DATA_DIR.glob("*.json") if p.stem >= since)
